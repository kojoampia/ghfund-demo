...
type RestOf<T extends IInvestment | NewInvestment> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class InvestmentService {
  ...
  protected convertDateFromClient<T extends IInvestment | NewInvestment | PartialUpdateInvestment>(investment: T): RestOf<T> {
    return {
      ...investment,
      date: investment.date?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvestment: RestInvestment): IInvestment {
    return {
      ...restInvestment,
      date: restInvestment.date ? dayjs(restInvestment.date) : undefined,
    };
  }
